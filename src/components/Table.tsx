  ...
  if (mobileRender) {
    return (
      <div className={`w-full ${className}`}>
        {/* Desktop View */}
        <div className="hidden md:block overflow-x-auto rounded-[12px] border border-borderCustom bg-surface">
        ...
        <div className="block md:hidden flex flex-col gap-3">
          {isLoading ? (
            <div className="py-12 text-center rounded-[12px] border border-borderCustom bg-surface">
              <Loader variant="inline" />
            </div>
          ) : data.length === 0 ? (
            <div className="py-12 rounded-[12px] border border-borderCustom bg-surface">
              {emptyState || (
                <div className="text-center text-textSecondary text-sm font-medium">
                  No data available.
                </div>
            ...
            data.map((row, rowIdx) => (
              <div key={row.id || rowIdx} className="rounded-[12px] border border-borderCustom bg-surface p-4">
                {mobileRender(row)}
              </div>
            ))
          )}
