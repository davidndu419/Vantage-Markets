...
import { initializeFirestore, doc, writeBatch, setDoc } from 'firebase/firestore';
...
// Force Long Polling to ensure the Auth Token propagates in Node.js
...
async function seed() {
  console.log('Authenticating test seeder account...');
  const email = 'testseeder@vantage.com';
  const password = 'TestSeeder123!';
  let uid = '';

  try {
    const cred = await createUserWithEmailAndPassword(auth, email, password);
    uid = cred.user.uid;
    console.log('Registered new test seeder user:', uid);
  } catch (err) {
    if (err.code === 'auth/email-already-in-use') {
      const cred = await signInWithEmailAndPassword(auth, email, password);
      uid = cred.user.uid;
      console.log('Logged in to existing test seeder user:', uid);
    } else {
      throw err;
    }
  }
  ...
  console.log('Starting Vantage Markets database seed under authenticated session...');
